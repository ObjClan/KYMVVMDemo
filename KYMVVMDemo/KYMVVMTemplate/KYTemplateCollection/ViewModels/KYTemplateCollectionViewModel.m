...
#import "KYTemplateCollectionCellSectionModel.h"
...
- (void)fetchDataWithIsRefresh:(BOOL)isRefresh
{
    ...
    if (isRefresh) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            KYTemplateCollectionCellSectionModel *section = [[KYTemplateCollectionCellSectionModel alloc] init];
            ...
            for (int i = 0; i < self.pageCount; i++) {
                KYTemplateCollectionCellItemModel *item = [[KYTemplateCollectionCellItemModel alloc] init];
                item.itemId = @"";
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            KYTemplateCollectionCellSectionModel *section = (KYTemplateCollectionCellSectionModel *)[self getSectionModelWithId:firstSectionId];
